...
class unet_down_sk_v2(unet_down_sk):
    def __init__(self, in_num=1, out_num=1, filters=[32, 64, 128, 128, 128, 128],
                 activation='sigmoid'):
        super().__init__(in_num, out_num, filters, activation)
...
class unet_down_sk_v3(unet_down_sk):
    def __init__(self, in_num=1, out_num=1, filters=[32, 64, 128, 128, 128, 128],
                 activation='sigmoid'):
        super().__init__(in_num, out_num, filters, activation)
    ...
    def forward(self, x):
        ...
        # side output 3
        so3_add = self.so3_conv1(x) + self.up(so4_add)
        so3 = self.so3_fconv(so3_add + self.down(self.down(self.down(z0))))
        so3 = F.sigmoid(so3)
        ...
        # side output 2
        so2_add = self.so2_conv1(x) + self.up(so3_add)
        so2 = self.so2_fconv(so2_add + self.down(self.down(z0)))
        so2 = F.sigmoid(so2)
        ...
        # side output 1
        so1_add = self.so1_conv1(x) + self.up(so2_add)
        so1 = self.so1_fconv(so1_add + self.down(z0))
        so1 = F.sigmoid(so1)
...
class unet_down_sk_v4(unet_down_sk):
    def __init__(self, in_num=1, out_num=1, filters=[32, 64, 128, 128, 128, 128],
                 activation='sigmoid'):
        super().__init__(in_num, out_num, filters, activation)
...
class unet_down_sk_v5(unet_down_sk):
    def __init__(self, in_num=1, out_num=1, filters=[32, 64, 128, 128, 128, 128],
                 activation='sigmoid'):
        super().__init__(in_num, out_num, filters, activation)
