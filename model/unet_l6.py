...
class unet_L6(nn.Module):
    def __init__(self, in_num=1, out_num=1, filters=[32,64,128,128,128,128], activation='sigmoid'):
        ...
        self.center = nn.Sequential(
            bottleneck_dilated(filters[4], filters[5], projection=True, dilate=2),
            bottleneck_dilated(filters[5], filters[5], projection=False,dilate=2),
            bottleneck_dilated(filters[5], filters[5], projection=True, dilate=2)
...
class unet_L6_sk(unet_L6):
    def __init__(self, in_num=1, out_num=1, filters=[32,64,128,128,128,128], 
        ...
        # no supervision
        self.so4_conv1 = conv2d_bn_elu(filters[4], filters[0], kernel_size=(3,3), padding=(1,1))
        self.so5_conv1 = conv2d_bn_elu(filters[5], filters[0], kernel_size=(3,3), padding=(1,1))
    ...
    def forward(self, x):
        ...
        if self.smooth==True:
            return x, so0, so1, so2, so3, so0_add, so1_add, so2_add, so3_add, so4_add, so5_add, map_pre
        else:    
            return x, so0, so1, so2, so3 

class unet_L6_ds(unet_L6):
    def __init__(self, in_num=1, out_num=1, filters=[32,64,128,128,128,128], 
                 activation='sigmoid', smooth=False):
        super().__init__(in_num, out_num, filters, activation)
        ...
        self.so1_conv1 = conv2d_bn_elu(filters[1], filters[0], kernel_size=(3,3), padding=(1,1))
        self.so1_fconv = conv2d_bn_elu(filters[0], out_num, kernel_size=(3,3), padding=(1,1))

        self.so2_conv1 = conv2d_bn_elu(filters[2], filters[0], kernel_size=(3,3), padding=(1,1))
        self.so2_fconv = conv2d_bn_elu(filters[0], out_num, kernel_size=(3,3), padding=(1,1))

        self.so3_conv1 = conv2d_bn_elu(filters[3], filters[0], kernel_size=(3,3), padding=(1,1))
        self.so3_fconv = conv2d_bn_elu(filters[0], out_num, kernel_size=(3,3), padding=(1,1))

        self.so4_conv1 = conv2d_bn_elu(filters[4], filters[0], kernel_size=(3,3), padding=(1,1))
        self.so4_fconv = conv2d_bn_elu(filters[0], out_num, kernel_size=(3,3), padding=(1,1))

        self.so5_conv1 = conv2d_bn_elu(filters[5], filters[0], kernel_size=(3,3), padding=(1,1))
        self.so5_fconv = conv2d_bn_elu(filters[0], out_num, kernel_size=(3,3), padding=(1,1))

        self.map_fconv = conv2d_bn_non(6, out_num, kernel_size=(3,3), padding=(1,1))    
